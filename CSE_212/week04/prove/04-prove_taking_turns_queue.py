...
# Defect(s) Found: 
# Players were getting to go again multiple times before the next players turn
...
while len(players) > 0:
    players.get_next_person()
# Defect(s) Found: 
# George in not being added into the queue properly beceause the loop is going through a range of 5
# which the first set of players are being added just like the first error. after 5 plays George is 
# getting to play all of his turns first cause an error. 
...
#print(players)    
# Defect(s) Found: 
# Instead of Tim getting to play forever he is only added once. There must be an error when the 
# player gets only 0 turns. Since there will only be 6 plays because of Tim only playing once 
# the loop will think there is nobody in the queue. Printing no one in the queue.
...
players.get_next_person()
# Defect(s) Found:
#  This one seams to be working correctly where when no one is in the queue we get an error message.

# When adding the items to the queue the code was adding them to 
# the begining instead of appending() them to the back (line 26).

# When there is a 0 for the turns of a player we were not having them go infinitly. To change this we 
# needed to add another if statement to check if the players turns = 0 and if it did they were keep 
# playing (line 120)